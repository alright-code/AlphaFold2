...
class Block(nn.Module):
    def __init__(self, c_m, a_m, heads_m, c_z, a_z, heads_z):
        super().__init__()
        ...
        self.a2 = ColWiseGatedAttention(c_m=c_m, num_heads=heads_m, c=a_m)
        self.trans1 = Transition(c=c_m)

        # Communication
        self.op_mean = OPMean(c_m=c_m, c_z=c_z, c=a_m)

        # Pair Stack
        self.tri1 = TriangleOutgoing(c_z=c_z, c=a_z)
        self.d2 = RowWiseDropout(0.25)
        self.tri2 = TriangleIncoming(c_z=c_z, c=a_z)
        self.tri3 = TriangleAttentionStart(c_z=c_z, num_heads=heads_z, c=a_z)
        self.tri4 = TriangleAttentionEnd(c_z=c_z, num_heads=heads_z, c=a_z)
        self.d3 = ColWiseDropout(0.25)
        self.trans2 = Transition(c=c_z)
...
class RowWiseGatedAttention(nn.Module):
    def __init__(self, c_m, num_heads, c_z, c):
        super().__init__()

        self.c = c
        self.num_heads = num_heads

        self.ln1 = nn.LayerNorm(c_m)  # b x numclust x cropsize x c
    ...
    def forward(self, msa_rep, pair_rep):
        # Input Projections
        msa_rep = self.ln1(msa_rep.permute(0, 2, 3, 1)).permute(
            0, 3, 1, 2
        )  # b x c_m x s x i
        ...
        b = self.w_b(
            self.ln2(pair_rep.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        )  # b x h x i x j

        g = torch.sigmoid(self.w_g(msa_rep))  # b x (c * h) x s x i
        g = einops.rearrange(
            g, "b (c h) s i -> b c h s i", h=self.num_heads
        )  # b x c x h x s x i

        # Attention
        q = einops.rearrange(
            q, "b (c h) s i -> b c h s i", h=self.num_heads
        )  # b x c x h x s x i
        k = einops.rearrange(
            k, "b (c h) s i -> b c h s i", h=self.num_heads
        )  # b x c x h x s x i
        v = einops.rearrange(
            v, "b (c h) s i -> b c h s i", h=self.num_heads
        )  # b x c x h x s x i

        a = torch.softmax(
            math.sqrt(self.c) ** -1 * torch.einsum("bchsi,bchsj->bhsij", q, k)
            + b.unsqueeze(2),
            dim=-1,
        )  # b x h x s x i x j

        o = g * torch.einsum("bhsij,bchsj->bchsi", a, v)  # b x c x h x s x i
...
class ColWiseGatedAttention(nn.Module):
    def __init__(self, c_m, num_heads, c):
        super().__init__()
    ...
    def forward(self, msa_rep):
        # Input projections
        msa_rep = einops.rearrange(msa_rep, "b c s i -> b c i s")
        ...
        a = torch.softmax(
            math.sqrt(self.c) ** -1 * torch.einsum("bchsi,bchsj->bhsij", q, k), dim=-1
        )  # b x h x s x i x j
        ...
        msa_rep = einops.rearrange(msa_rep, "b c i s -> b c s i")
...
class Transition(nn.Module):
    ...
    def forward(self, msa_rep):
        msa_rep = self.ln1(msa_rep.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        a = self.l1(msa_rep)
...
class OPMean(nn.Module):
    ...
    def forward(self, msa_rep):
        ...
        o = torch.mean(torch.einsum("bcsi,bvsj->bcvsij", a, b), dim=3).flatten(
            1, 2
        )  # b x (c * c) x i x j
...
class TriangleOutgoing(nn.Module):
    ...
    def forward(self, pair_rep):
        pair_rep = self.ln1(pair_rep.permute(0, 2, 3, 1)).permute(
            0, 3, 1, 2
        )  # b x c_z x i x j
        ...
        pair_rep = g * self.w_rep(
            self.ln2(torch.einsum("bcik,bcjk->bcij", a, b).permute(0, 2, 3, 1)).permute(
                0, 3, 1, 2
            )
        )  # b x c_z x i x j
...
class TriangleIncoming(nn.Module):
    ...
    def forward(self, pair_rep):
        ...
        pair_rep = g * self.w_rep(
            self.ln2(torch.einsum("bcki,bckj->bcij", a, b).permute(0, 2, 3, 1)).permute(
                0, 3, 1, 2
            )
        )  # b x c_z x i x j
...
class TriangleAttentionStart(nn.Module):
    def __init__(self, c_z, num_heads, c):
        super().__init__()
    ...
    def forward(self, pair_rep):
        # Input Projections
        pair_rep = self.ln1(pair_rep.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        ...
        g = einops.rearrange(
            g, "b (h c) i j -> b c h i j", h=self.num_heads
        )  # b x c x h x i x j

        # Attention
        q = einops.rearrange(
            q, "b (h c) i j -> b c h i j", h=self.num_heads
        )  # b x c x h x i x j
        k = einops.rearrange(
            k, "b (h c) i j -> b c h i j", h=self.num_heads
        )  # b x c x h x i x j
        v = einops.rearrange(
            v, "b (h c) i j -> b c h i j", h=self.num_heads
        )  # b x c x h x i x j

        a = torch.softmax(
            math.sqrt(self.c) ** -1 * torch.einsum("bchij,bchik->bhijk", q, k)
            + b.unsqueeze(2),
            dim=-1,
        )  # b x h x i x j x k

        o = g * torch.einsum("bhijk,bchik->bchij", a, v)  # b x c x h x i x j
...
class TriangleAttentionEnd(nn.Module):
    ...
    def forward(self, pair_rep):
        ...
        a = torch.softmax(
            math.sqrt(self.c) ** -1 * torch.einsum("bchij,bchkj->bhijk", q, k) + b,
            dim=-1,
        )  # b x h x i x j x k

        o = g * torch.einsum("bhijk,bchkj->bchij", a, v)  # b x c x h x i x j
